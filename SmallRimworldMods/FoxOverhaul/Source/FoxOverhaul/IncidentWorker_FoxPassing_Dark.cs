...
{
    public class IncidentWorker_FoxPassing_Dark : IncidentWorker
    {
        protected override bool CanFireNowSub(IncidentParms parms)
        {
            Map map = (Map)parms.target;
            IntVec3 cell;
            return !map.gameConditionManager.ConditionIsActive(GameConditionDefOf.ToxicFallout) && map.mapTemperature.SeasonAndOutdoorTemperatureAcceptableFor(ThingDef.Named("NBF_Fox_DarkRace"), 0f) && this.TryFindEntryCell(map, out cell);
        }
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            ...
            {
                PawnKindDef pawnKindDef = PawnKindDef.Named("NBF_Fox_Dark");
                ...
                if (flag4)
                {
                    ...
                    base.SendStandardLetter(TranslatorFormattedStringExtensions.Translate("LetterLabelDarkFoxPassing", pawnKindDef.label).CapitalizeFirst(), TranslatorFormattedStringExtensions.Translate("LetterDarkFoxPassing", pawnKindDef.label), LetterDefOf.PositiveEvent, parms, pawn, Array.Empty<NamedArgument>());
                    flag2 = true;
                }
                else
                {
